...
const COMPARISON = [
  ...
  { feature: "Disponible en 2 tailles",               ours: true,  classic: false },
];

const FAQ_ITEMS = [
  {
    q: "Quelle fermeté choisir : moelleux ou ferme ?",
    a: "Si vous dormez principalement sur le côté, optez pour le modèle ferme qui maintient mieux la nuque dans l'alignement. Si vous dormez sur le dos ou changez souvent de position, le modèle moelleux offrira un confort plus enveloppant. En cas de doute, le moelleux convient à la majorité des dormeurs.",
  },
  {
...
export default function OreilleMemoireFormePage() {
  ...
  function handleOrder(e: React.FormEvent) {
    ...
    saveLastOrder({
      ...
      customer: { ...form, notes: `Modèles : ${modelSummary}` },
      total: totalPrice,
          ...
          <div className="flex flex-col gap-6">
            <div>
              ...
              <p className="mt-3 text-gray-500 leading-relaxed">
                Mousse viscoélastique haute densité, housse anti-acariens en microfibre douce.
                Disponible en fermeté moelleux ou ferme, pour un sommeil sur mesure.
              </p>
              <div className="mt-3 flex items-center gap-2">
                {[1, 2, 3, 4, 5].map((s) => (
                  <Star key={s} size={16} className="fill-yellow-400 text-yellow-400" />
                ))}
                <span className="text-sm text-gray-500">4.8 / 5 — 200+ avis</span>
              </div>
            </div>

            {/* Offers + model dropdowns */}
            <div>
